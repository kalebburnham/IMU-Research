...
[Acc, Gyr] = addNoise(sampleSize, Acc, Gyr);
[Acc, Gyr] = addBias(sampleSize, Acc, Gyr);
...
figure;
hold on;
plot(1024:1105, GlobalAcc(1024:1105,3), 'r');
plot(1024:1105, GroundTruthAcceleration(1024:1105,3), 'g');
hold off;
...
figure;
scatter(1:100, GlobalAcc(1024:1123,3))



function [Acc,Gyr] = addNoise(sampleSize, Acc, Gyr)
    % Add white noise. Acc = 0.012 m/s/s, Gyr = 0.0087 rad/s
    mean = 0;
    AccSigma = 0.012; % Std Dev.
    GyrSigma = 0.0087;
    AccRandomWalk = zeros(size(Acc));
    GyrRandomWalk = zeros(size(Gyr));
    
    AccRandomWalk(1,:) = normrnd(mean,AccSigma,[1 3]);
    GyrRandomWalk(1,:) = normrnd(mean,GyrSigma,[1 3]);
    
    for t = 2:sampleSize
        AccRandomWalk(t,:) = AccRandomWalk(t-1,:) + normrnd(mean,AccSigma,[1,3]);
        GyrRandomWalk(t,:) = GyrRandomWalk(t-1,:) + normrnd(mean,GyrSigma,[1,3]);
    end
    
    Acc = Acc + AccRandomWalk;
    Gyr = Gyr + GyrRandomWalk;
end

function [Acc,Gyr] = addBias(sampleSize, Acc,Gyr)
